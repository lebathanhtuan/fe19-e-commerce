...
function* getCartListSaga(action) {
  try {
    ...
    const result = yield axios.get("https://fe19-e-commerce-api.herokuapp.com/carts", {
      params: {
        userId,
        _expand: "product",
      },
...
function* addToCartSaga(action) {
  try {
    ...
    const result = yield axios.post(
      `https://fe19-e-commerce-api.herokuapp.com/carts`,
      action.payload
    );
    yield put({
...
function* updateCartProductSaga(action) {
  try {
    ...
    yield axios.patch(`https://fe19-e-commerce-api.herokuapp.com/carts/${data.id}`, {
...
function* removeCartProductSaga(action) {
  try {
    ...
    yield axios.delete(`https://fe19-e-commerce-api.herokuapp.com/carts/${id}`);
    yield put({
