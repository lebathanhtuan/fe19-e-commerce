...
function* getProductListSaga(action) {
  ...
  try {
    ...
    const result = yield axios.get(
      `https://fe19-e-commerce-api.herokuapp.com/products?${categoryParam}`,
      {
        params: {
          _limit: limit,
          _page: page,
...
function* getProductDetailSaga(action) {
  try {
    ...
    const result = yield axios.get(
      `https://fe19-e-commerce-api.herokuapp.com/products/${id}?_embed=productOptions&_embed=favorites`
    );
    yield put({
...
function* createProductSaga(action) {
  try {
    ...
    yield axios.post(
      "https://fe19-e-commerce-api.herokuapp.com/products",
      data
    );
    yield put({
...
function* updateProductSaga(action) {
  try {
    ...
    yield axios.patch(
      `https://fe19-e-commerce-api.herokuapp.com/products/${id}`,
      data
    );
    yield put({
