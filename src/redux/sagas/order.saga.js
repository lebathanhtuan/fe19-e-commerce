...
function* getOrderListSaga(action) {
  try {
    ...
    const result = yield axios.get(
      `https://fe19-e-commerce-api.herokuapp.com/orders`,
      {
        params: {
          userId: id,
          _order: "desc",
...
function* orderCartSaga(action) {
  try {
    ...
    yield axios.post("https://fe19-e-commerce-api.herokuapp.com/orders", data);
    yield data.products.forEach((productItem) => {
      axios.delete(
        `https://fe19-e-commerce-api.herokuapp.com/carts/${productItem.cartId}`
      );
    });
